...
| `BLOB_READ_WRITE_TOKEN` | No (required on Vercel/serverless) | Uploads use Vercel Blob when set, local disk otherwise — see the caveat below. |

## One real caveat, not glossed over

**Uploads use Vercel Blob when configured, local disk otherwise.** `/api/uploads` writes into `public/uploads` on the container's filesystem UNLESS the `BLOB_READ_WRITE_TOKEN` env var is set, in which case it uploads to Vercel Blob instead — that token works from any host, not just Vercel (create a Blob store in the Vercel dashboard, copy its token here even if the app itself runs on Render). Local disk remains fine for a single long-running server with a persistent volume mounted at `public/uploads`, but is NOT viable on Vercel or most other serverless hosts — their filesystem is read-only outside `/tmp`, so an upload attempt fails outright rather than just not persisting. Set `BLOB_READ_WRITE_TOKEN` before deploying anywhere with an ephemeral/read-only filesystem.
