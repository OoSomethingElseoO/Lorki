...
type ArtistNewCampaignFormProps = {
  animals: { id: string; name: string; species: string; conservancyName: string }[];
  conservancies: { id: string; name: string }[];
};

export function ArtistNewCampaignForm({ animals, conservancies }: ArtistNewCampaignFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/artist/campaigns", {
      method: "POST",
    ...
    router.push(`/artist/artworks/new?campaignId=${data.campaign.id}`);
  }
