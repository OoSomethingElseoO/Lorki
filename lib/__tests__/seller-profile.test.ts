// app/api/seller/profile/route.ts (PATCH) gates on getCurrentUser(), which
...
test("seller profile PATCH updates name, country, bio, and imageUrl", async (t) => {
  const artist = await createArtist();
  t.after(async () => {
    await prisma.artist.delete({ where: { id: artist.id } });
  });

  // Exactly the data object app/api/seller/profile/route.ts's PATCH writes.
...
test("seller profile PATCH's update never touches slug, even though the route accepts a new name", async (t) => {
  const artist = await createArtist();
  t.after(async () => {
    await prisma.artist.delete({ where: { id: artist.id } });
  });
  ...
  // verbatim (compare against app/api/seller/profile/route.ts's PATCH).
  const updated = await prisma.artist.update({
    where: { id: artist.id },
    data: {
