// app/api/artist/artworks/[id]/route.ts (PATCH, DELETE) gates on
...
// artist-profile.test.ts). What's genuinely valuable and independent of
...
async function createArtist(label: string) {
  ...
  return prisma.artist.create({
    data: {
      ...
      name: `Test ${label}`,
      country: "Kenya",
      bio: "A throwaway artist created by artist-artworks.test.ts",
...
async function createArtwork(artistId: string, inventoryState: "AVAILABLE" | "SOLD") {
  ...
  const artwork = await prisma.artwork.create({
    data: {
      ...
      altText: "A throwaway artwork created by artist-artworks.test.ts",
      inventoryState,
    },
  });
...
// Reproduces app/api/artist/artworks/[id]/route.ts's loadOwnArtwork verbatim.
...
test("loadOwnArtwork loads an artwork whose campaign belongs to the requesting artist", async (t) => {
...
test("loadOwnArtwork returns null for an artwork belonging to a different artist", async (t) => {
  ...
  assert.equal(owned, null, "a different artist must never load this artwork");
});
