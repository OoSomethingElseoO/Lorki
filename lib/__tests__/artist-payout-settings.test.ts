// app/api/artist/payout-settings/route.ts (PATCH) gates on
// getCurrentUser(), so the direct-handler-call pattern doesn't work (same
// situation as artist-profile.test.ts / artist-artworks.test.ts). Its
...
// Reproduces app/api/artist/payout-settings/route.ts's PATCH validation
