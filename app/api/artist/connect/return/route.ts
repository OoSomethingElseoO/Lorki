...
export async function GET(request: Request) {
  ...
  if (!artist?.stripeConnectedAccountId) {
    return NextResponse.redirect(`${origin}/artist/profile`);
  }

  try {
    const stripe = await getStripe();
    const account = await stripe.accounts.retrieve(artist.stripeConnectedAccountId);
    await prisma.artist.update({
      where: { id: artist.id },
      data: { stripeConnectOnboarded: Boolean(account.payouts_enabled) },
    });
  } catch (error) {
    console.error("[artist/connect/return] Failed to refresh Connect account status", error);
  }
